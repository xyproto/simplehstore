...
func TestJMapShort(t *testing.T) {
	Verbose = true
	host := NewHost(defaultConnectionString)
	defer host.Close()

	jm, err := NewJMap(host, jmapname)
	if err != nil {
		t.Error(err)
	}
	...
	err = jm.Set(username, "b", "true")
	if err != nil {
		t.Error(err)
	}

	err = jm.SetMap(username, map[string]string{"x": "42", "y": "64"})
	if err != nil {
		t.Error(err)
	}

	err = jm.Set(username, "y", "128")
	if err != nil {
		t.Error(err)
	}
	...
	err = jm.Remove()
	if err != nil {
		t.Errorf("Error, could not remove jmap! %s", err)
	}
}

func TestJMapUserStateShort(t *testing.T) {
	Verbose = true
	...
	hashmap, err := NewJMap(host, hashmapname)
	if err != nil {
		t.Error(err)
	}
...
func TestJMapUserState(t *testing.T) {
	Verbose = true
...
func TestJMapUserState2(t *testing.T) {
	Verbose = true
...
func TestDashesAndQuotes2(t *testing.T) {
	...
	defer host.Close()
	hashmap, err := NewJMap(host, hashmapname+"'s-")
	if err != nil {
		t.Error(err)
	}
	hashmap.Clear()
